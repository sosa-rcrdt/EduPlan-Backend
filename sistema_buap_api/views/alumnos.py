...
class AlumnosAll(generics.CreateAPIView):
    ...
    # Devuelve el listado de alumnos activos
...
class AlumnosView(generics.CreateAPIView):
    permission_classes = (permissions.IsAuthenticated,)
    # Obtiene la información de un alumno por id
    def get(self, request, *args, **kwargs):
        alumno = get_object_or_404(Alumnos, id=request.GET.get("id"))
        alumno = AlumnoSerializer(alumno, many=False).data
        return Response(alumno, 200)

    # Registra un nuevo alumno y su usuario asociado
    @transaction.atomic
    def post(self, request, *args, **kwargs):
        ...
        if user.is_valid():
            ...
            # Crear perfil de alumno asociado
            alumno = Alumnos.objects.create(
                user=user,
                matricula=request.data["matricula"],
                curp=request.data["curp"].upper(),  # se guarda CURP en mayúsculas
                rfc=request.data["rfc"].upper(),    # se guarda RFC en mayúsculas
...
class AlumnosViewEdit(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        user = AlumnoSerializer(alumno, many=False).data  # se deja por si se usa después
        return Response({"alumno_updated_id": alumno.id}, 200)

    # Elimina al alumno y su usuario asociado
